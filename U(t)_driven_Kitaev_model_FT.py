...
def kitaev_ft_Hamiltonian_FT_OBC(kx, Jx, Jy, Jz, a0, N):
    ...
    bigH[0:2, -2:] = 0
    bigH[-2:, 0:2] = 0
    #bigH = np.where(np.abs(bigH.real) < threshold, 0, bigH.real) + 1j * np.where(np.abs(bigH.imag) < threshold, 0, bigH.imag)
    return bigH
delta = 0 # testing 
N = 100
...
epsilon_T = -np.angle(eigvals)  # Shape (len(kx), 2*N)
...
plt.title('Floquet Quasi-energy Bands Driven Kitaev Model (perturbed, delta=0.66, N=40)')
